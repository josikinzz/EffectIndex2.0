...
const ADMIN_EMAIL = 'admin@effectindex.local';
...
const firstRun = async function() {
  ...
  if (users.length === 0) {
    ...
    ${chalk.green('Email:')} ${chalk.keyword("yellow")(ADMIN_EMAIL)}
    ${chalk.white('Be sure to create real admin credentials if using this in production.')}`);

    const hash = await bcrypt.hash(ADMIN_PASSWORD, 10);
    let newUser = new User({ username: ADMIN_USERNAME, email: ADMIN_EMAIL, hash, roles: ['admin'] });
