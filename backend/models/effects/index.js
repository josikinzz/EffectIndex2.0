...
router.post('/', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('admin-effects'), async (req, res, next) => {
  try {
...
router.get('/admin/:slug', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('admin-effects', 'edit-effects'), async (req, res, next) => {
  try {
...
router.post('/:id', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('edit-effects', 'admin-effects'), async (req, res) => {
  const { name, description, summary, long_summary, analysis, style_variations, personal_commentary,
  contributors, external_links, see_also, tags, citations, gallery_order, social_media_image,
...
router.delete('/:id', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('admin-effects'), async (req, res) => {
  try {
