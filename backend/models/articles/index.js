...
router.post('/', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('all-articles', 'own-articles'), async (req, res, next) => {
  try {
...
router.post('/:id', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('own-articles', 'all-articles'), async (req, res, next) => {
  try {
    const {id} = req.params;
...
router.delete('/:id', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('all-articles', 'own-articles'), async (req, res, next) => {
  try {
    const {id} = req.params;
    const {user} = req;
...
router.get('/admin', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('own-articles', 'all-articles'), async (req, res, next) => {
  try {
    const {user} = req;
    let articles;
...
router.get('/admin/:_id', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('own-articles', 'all-articles'), async (req, res, next) => {
  try {
