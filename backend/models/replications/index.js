...
router.post('/', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('all-replications', 'own-replications'), async (req, res, next) => {
  try {
...
router.get('/', async (req, res, next) => {
  try {
    const replications = await Replication.find()
      .select('-user')
      .populate({
        path: 'person',
...
router.post('/:id', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256'] }), hasPerms('all-replications', 'own-replications'), async (req, res, next) => {
  try {
    const { user } = req;
...
router.delete('/:_id', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256'] }), hasPerms('all-replications', 'own-replications'), async (req, res, next) => {
  try {
    const { user } = req;
