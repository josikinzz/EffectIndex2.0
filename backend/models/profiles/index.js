...
router.post('/imageUpload', secured({ requestProperty: 'user', secret, algorithms: ['HS256'] }), hasPerms('admin'), uploadAny, async(req, res, next) => {
  try {
    const { files } = req;
...
router.post('/', secured({ requestProperty: 'user', secret, algorithms: ['HS256'] }), hasPerms('admin'), async(req, res, next) => {
  try {
...
router.put('/:id', secured({ requestProperty: 'user', secret, algorithms: ['HS256'] }), hasPerms('admin'), uploadAny, async(req, res, next) => {
...
router.delete('/:id', secured({ requestProperty: 'user', secret, algorithms: ['HS256'] }), hasPerms('admin'), async(req, res, next) => {
  let id = req.params.id;
  try {
...
router.get('/:id', secured({ requestProperty: 'user', secret, algorithms: ['HS256'] }), hasPerms('admin'), async(req, res, next) => {
  let id = req.params.id;
  try {
