...
router.post('/', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('manage-blog'), async (req, res, next) => {
  try {
...
router.post('/:id', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('manage-blog'), async(req, res, next) => {
  try {
...
router.get('/:id/delete', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('manage-blog'), async(req, res, next) => {
  try {
