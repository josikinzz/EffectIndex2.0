...
router.get('/', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('admin'), async (req, res, next) => {
  try {
...
router.post('/add', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('admin'), async (req, res, next) => {
...
router.post('/changePassword', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), async (req, res, next) => {
  try {
...
router.get('/user', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), async (req, res, next) => {
  try {
    const { user } = req;
    if (user && '_id' in user) {
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    const token = jwt.sign({ username: foundUser.username, _id: foundUser._id, roles: foundUser.roles }, config.jwtSecret);
    res.send({ token });
  } catch (error) {
    next(error);
  }
...
router.get('/:_id', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('admin'), async (req, res, next) => {
...
router.post('/:_id', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('admin'), async(req, res, next) => {
...
router.delete('/:_id', secured({ requestProperty: 'user', secret: config.jwtSecret, algorithms: ['HS256']}), hasPerms('admin'), async (req, res, next) => {
  const { _id } = req.params;
